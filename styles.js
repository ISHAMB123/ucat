...
.ud {
  ...
  --signal:#F5A524; --go:#3ECF8E; --stop:#F2555A; --mute:#8794A5; --sea:#2F71B8;
  --body:#C0CAD6; --card:#EDEFF2;
  /* Cards/panels are light in BOTH themes, so their strong text needs a colour
     that stays dark in both. --ink is the page background and flips to near
     white in light mode, so it must never be used as text on a card. */
  --card-ink:#111820;
...
.ud.light {
  ...
  --signal:#D98E00; --go:#1E8E5A; --stop:#D64545; --mute:#5C6B7C; --sea:#2F71B8;
...
.auth-home b { color:var(--card-ink); }
...
.ud-sub.sub-feature .sub-ico { background:var(--signal); color:var(--card-ink); }
...
.rd-p-head span:first-of-type, .rd-p-head span { color:var(--card-ink); text-transform:none; letter-spacing:0; font-size:15px; font-weight:600; }
...
.logs-name { font-size:14px; font-weight:600; color:var(--card-ink); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
...
.auth-tabs button.on { background:var(--card); color:var(--card-ink); box-shadow:0 1px 3px rgba(16,24,32,0.12); }
...
/* master (owner) account warning bar */
.master-banner { position:fixed; top:0; left:0; right:0; z-index:61; height:46px; display:flex; align-items:center; justify-content:center; gap:10px; padding:0 16px;
...
.res-weak h3 { font-size:16px; margin:8px 0 6px; color:var(--card-ink); }
...
.rev-q { font-size:14px; margin:0 0 6px; color:var(--card-ink); }
...
.sc-legend .k b { color:var(--card-ink); }
...
.li-fmt.on .li-fmt-ico { background:var(--signal); color:var(--card-ink); }
...
.li-done-ic { width:40px; height:40px; flex:none; border-radius:11px; background:var(--go); color:var(--card-ink); display:grid; place-items:center; }
...
.li-tag { position:absolute; top:-9px; left:50%; transform:translateX(-50%); white-space:nowrap; font-family:'Inter',sans-serif; font-size:8.5px; letter-spacing:0.1em; text-transform:uppercase; background:var(--signal); color:var(--card-ink); border-radius:20px; padding:3px 9px; font-weight:600; }
...
.li-stage-row { display:flex; align-items:center; gap:22px; justify-content:center; max-width:680px; margin:6px auto 0; }
.li-doc-wrap { flex:none; display:flex; flex-direction:column; align-items:center; gap:8px; }
.li-doc { position:relative; width:104px; height:104px; border-radius:50%; overflow:hidden; background:radial-gradient(120% 120% at 50% 22%, color-mix(in srgb, var(--signal) 16%, var(--card)), var(--card)); border:1px solid var(--paperline); box-shadow:0 8px 22px -14px rgba(16,24,32,.5); animation:li-bob 3.4s ease-in-out infinite; }
.li-doc svg { position:absolute; left:50%; top:12px; width:96px; height:108px; transform:translateX(-50%); display:block; }
.li-doc-cap { display:inline-flex; align-items:center; gap:6px; font-family:'Inter',sans-serif; font-size:10px; font-weight:600; letter-spacing:0.14em; text-transform:uppercase; color:var(--mute); }
.li-doc-live { width:6px; height:6px; border-radius:50%; background:var(--go); animation:li-blink 1s infinite; }
.li-doc-mouth { transform-box:fill-box; transform-origin:center; }
.li-doc.talk .li-doc-mouth { animation:li-talk .26s ease-in-out infinite; }
.li-doc.think { animation:li-bob 3.4s ease-in-out infinite, li-tilt 1.6s ease-in-out infinite; }
@keyframes li-bob { 0%,100% { transform:translateY(0); } 50% { transform:translateY(-4px); } }
@keyframes li-tilt { 0%,100% { rotate:0deg; } 50% { rotate:-4deg; } }
@keyframes li-talk { 0%,100% { transform:scaleY(0.45); } 50% { transform:scaleY(2.1); } }
.li-bubble { position:relative; flex:1; background:var(--card); border:1px solid var(--paperline); border-radius:16px; padding:18px 22px; font-family:'Bricolage Grotesque',sans-serif; font-size:20px; line-height:1.4; letter-spacing:-0.015em; color:#16202B; box-shadow:0 6px 18px -12px rgba(16,24,32,.35); }
.li-bubble::before { content:""; position:absolute; left:-9px; top:50%; transform:translateY(-50%); width:0; height:0; border:9px solid transparent; border-right-color:var(--paperline); }
.li-bubble::after { content:""; position:absolute; left:-7px; top:50%; transform:translateY(-50%); width:0; height:0; border:8px solid transparent; border-right-color:var(--card); }
@media (max-width:560px){ .li-stage-row { flex-direction:column; gap:12px; } .li-doc { width:86px; height:86px; } .li-doc svg { width:80px; height:90px; top:10px; } .li-bubble::before,.li-bubble::after { display:none; } }
...
.li-score { display:flex; align-items:center; gap:24px; border:1px solid var(--line); border-radius:10px; background:var(--slate); padding:20px 24px; margin-bottom:12px; }
.li-score-ringwrap { flex:none; display:flex; flex-direction:column; align-items:center; gap:9px; }
.li-score-ring { --p:0; width:118px; height:118px; border-radius:50%; background:conic-gradient(var(--go) calc(var(--p)*1%), color-mix(in srgb, var(--mute) 22%, transparent) 0); display:grid; place-items:center; position:relative; }
.li-score-ring::before { content:""; position:absolute; inset:11px; border-radius:50%; background:var(--slate); }
.li-score-ring b { position:relative; display:flex; align-items:baseline; font-family:'Bricolage Grotesque',sans-serif; font-size:34px; letter-spacing:-0.03em; color:var(--paper); line-height:1; }
.li-score-ring b i { font-size:16px; font-style:normal; color:var(--mute); margin-left:1px; }
.li-score-cap { font-family:'Inter',sans-serif; font-size:9.5px; letter-spacing:0.16em; text-transform:uppercase; color:var(--mute); }
.li-score-side { min-width:0; }
.li-score-verdict { display:block; font-family:'Bricolage Grotesque',sans-serif; font-size:25px; letter-spacing:-0.02em; color:var(--paper); }
...
.li-score-pips i { width:24px; height:6px; border-radius:4px; background:color-mix(in srgb, var(--mute) 22%, transparent); }
...
.li-dna { border:1px solid var(--line); border-radius:10px; background:var(--slate); padding:18px 20px; }
.li-dna-h { display:block; font-family:'Inter',sans-serif; font-size:9.5px; letter-spacing:0.14em; text-transform:uppercase; color:var(--signal); margin-bottom:16px; }
.li-dna-cols { display:flex; align-items:flex-end; justify-content:space-between; gap:12px; }
.li-dna-col { flex:1; display:flex; flex-direction:column; align-items:center; gap:9px; min-width:0; }
.li-dna-cv { font-family:'JetBrains Mono',monospace; font-size:15px; font-weight:700; color:var(--paper); line-height:1; }
.li-dna-cv i { font-size:9.5px; font-style:normal; color:var(--mute); }
.li-dna-shaft { width:24px; height:118px; border-radius:7px; background:color-mix(in srgb, var(--mute) 15%, transparent); display:flex; align-items:flex-end; overflow:hidden; }
.li-dna-shaft b { display:block; width:100%; border-radius:7px; background:var(--signal); transition:height .6s ease; min-height:7px; }
.li-dna-cl { font-family:'Inter',sans-serif; font-size:10px; line-height:1.25; color:var(--body); text-align:center; }
.li-dna-note { font-size:11.5px; color:var(--mute); margin:14px 0 0; line-height:1.5; }
...
.rg-k-scale i { width:94px; height:8px; border-radius:4px; background:linear-gradient(90deg,#2563AD,#2E9AC4,#2EBE98,#7AC94F,#DED23F,#F5A82C,#ED742D,#E33A34); }
...
.rg-k-ev i { width:18px; height:10px; border-radius:3px; background:rgba(34,197,94,0.32); border-left:3px solid #16A34A; }
.rg-advice { margin:13px 0 0; display:flex; flex-direction:column; gap:8px; }
.rg-advice p { margin:0; font-size:13px; line-height:1.55; color:var(--body); background:color-mix(in srgb, var(--go) 12%, transparent); border-left:3px solid var(--go); border-radius:0 8px 8px 0; padding:9px 12px; }
.rg-tech { margin:12px 0 0; font-size:12.5px; line-height:1.55; color:var(--body); background:color-mix(in srgb, var(--go) 10%, transparent); border-left:3px solid var(--go); border-radius:0 8px 8px 0; padding:9px 12px; }
.rg-tech b { color:var(--card-ink); text-transform:capitalize; }
